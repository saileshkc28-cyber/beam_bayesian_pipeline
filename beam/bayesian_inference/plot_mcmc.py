...
def autocorrelation(samples, maximum_lag):
    """Calculate the sample autocorrelation up to maximum_lag."""
    samples = np.asarray(
        samples,
        dtype=float
    )
...
def split_rhat(chains):
    ...
    number_of_chains, number_of_draws = (
        chains.shape
    )
...
def create_zone_figure(
    ...
    """Create trace, posterior, running-mean and ACF panels."""
    number_of_chains, number_of_draws = (
        chains.shape
    )

    draw_numbers = np.arange(
        1,
        number_of_draws + 1
    )
    ...
    trace_axis.set_xlabel("Stored draw")
    trace_axis.set_ylabel(r"$\alpha$")
    ...
    secondary_axis = (
        posterior_axis.secondary_xaxis(
            "top",
            functions=(
                lambda alpha: (
                    alpha
    ...
    running_axis.set_ylabel(
        r"Running mean of $\alpha$"
    )
    running_axis.set_title(
    ...
    figure.suptitle(
        (
            f"MCMC diagnostics — parameter {zone_index + 1}\n"
            f"split R-hat = {rhat:.4f}, "
            f"mean acceptance = "
            f"{np.mean(acceptance_rates):.3f}"
        ),
        fontsize=13
    )
