...
FIELDS = ["run", "label", "n_sensors", "stations", "n_eff", "sigma_assumed",
...
def collect(folder):
    ...
    return {
        ...
        "alpha_ls_from_data": info.get("alpha_ls_from_data"),
...
def figure_posterior(rows, out):
    ...
    span = hi - lo
    ax.set_xlim(lo - 0.09 * span, hi + 0.62 * span)
    for p, r in zip(pos, rows):
        mark = "" if r["covers_harm"] else "   *"
        ax.text(hi + 0.03 * span, p,
                f"{r['alpha_mean']:.4f} $\\pm$ {r['alpha_std']:.4f}   "
                f"pop {r['bias_pop_pct']:+.2f}%   harm {r['bias_harm_pct']:+.2f}%{mark}",
                va="center", ha="left", fontsize=8.5, color=EDGE)
    ...
    ax.set_title(f"Mean-only posterior vs. sensor count   |   sigma {SIGMA:.4e} per sensor"
                 f"   |   * = 95% CI misses harmonic mean", fontsize=12, pad=14)
    ax.grid(axis="x", alpha=0.25)
    ax.legend(handles=[
        Line2D([], [], color=TRUTH, ls="--", lw=1.8,
               label=f"population mean  {pop:.5f}"),
        Line2D([], [], color=HARM, ls=":", lw=2.0,
               label=f"harmonic mean  {harm:.5f}"),
...
def main():
    ...
    print(f"truth: population mean {pop:.6f}   harmonic mean {harm:.6f}\n")
    header = (f"{'run':<6}{'k':>4}{'N_eff':>8}{'alpha':>9}{'sd':>9}{'SD/SD1':>8}"
              f"{'pred':>7}{'LS':>9}{'pop%':>8}{'z_pop':>7}{'harm%':>8}{'z_harm':>7}"
              f"{'95% CI':>19}{'cov p/h':>9}{'lvls':>5}{'solves':>7}{'time_s':>8}")
    print(header)
    print("-" * len(header))
    for r in rows:
        ci = f"[{r['alpha_p2.5']:.4f},{r['alpha_p97.5']:.4f}]"
        ls = "" if r["alpha_ls_from_data"] is None else format(r["alpha_ls_from_data"], ">9.4f")
        ...
        print(f"{r['label']:<6}{r['n_sensors']:>4}{r['n_eff']:>8.2f}{r['alpha_mean']:>9.4f}"
              f"{r['alpha_std']:>9.4f}{r['sd_ratio']:>8.3f}{r['sd_pred_ratio']:>7.3f}{ls:>9}"
              f"{r['bias_pop_pct']:>8.2f}{r['z_pop']:>7.2f}"
              f"{r['bias_harm_pct']:>8.2f}{r['z_harm']:>7.2f}{ci:>19}{cov:>9}"
