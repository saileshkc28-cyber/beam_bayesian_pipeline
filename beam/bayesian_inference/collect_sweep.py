...
def main():
    ...
    is_noise = len({r["sigma_assumed"] for r in rows}) > 1
    is_prior = (not is_noise) and len({r["prior_parameters"] for r in rows}) > 1
    if is_noise:
    ...
    show_scaled = (not is_prior) and (not is_noise) and any(
